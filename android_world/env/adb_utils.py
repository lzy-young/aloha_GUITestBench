...
_PATTERN_TO_ACTIVITY = immutabledict.immutabledict({
    ...
    'ankidroid-2_8_2 | ankidroid-2_15 | ankidroid-2_14 | ankidroid-2_13_5': 'com.ichi2.anki/.IntentHandler', 
...
def get_adb_activity(app_name: str) -> Optional[str]:
  ...
  # 创建一个按长度排序的模式列表，长的（更具体）的模式优先匹配
  sorted_patterns = sorted(_PATTERN_TO_ACTIVITY.items(), key=lambda x: len(x[0]), reverse=True)
  
  # 首先尝试精确匹配，优先匹配更具体的模式
  for pattern, activity in sorted_patterns:
    if re.match(pattern.lower(), app_name.lower()):
      return activity
  
  # 预处理输入的 app_name，用于模糊匹配
  # 将空格、下划线、连字符替换为统一的分隔符，便于匹配
  processed_app_name = re.sub(r'[\s_\-]+', '.*', app_name.lower())
  
  # 尝试匹配处理后的模式
  for pattern, activity in sorted_patterns:
    processed_pattern = re.sub(r'[\s_\-]+', '.*', pattern.lower())
    # 检查处理后的名称是否匹配处理后的模式
    if re.search(processed_pattern, processed_app_name) or re.search(processed_app_name, processed_pattern):
      return activity
  
  # 如果精确和处理后的匹配都失败，尝试模糊匹配
  app_name_lower = app_name.lower()
  for pattern, activity in sorted_patterns:
    # 使用 difflib 进行模糊匹配，阈值设为 0.6
    similarity = difflib.SequenceMatcher(None, app_name_lower, pattern.lower()).ratio()
    if similarity > 0.6:  # 如果相似度超过60%
      return activity
